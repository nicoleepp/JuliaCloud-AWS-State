...
using .AWSCorePrototypeServices: simpledb
...
"""
DeleteAttributes(DomainName, ItemName) = simpledb("POST", "/")
...
"""
GetAttributes(DomainName, ItemName) = simpledb("POST", "/")
...
"""
DeleteDomain(DomainName) = simpledb("POST", "/")
...
"""
BatchDeleteAttributes(DomainName, Items) = simpledb("POST", "/")

"""
 Returns information about the domain, including when the domain was created, the number of items and attributes in the domain, and the size of the attribute names and values. 
"""
DomainMetadata(DomainName) = simpledb("POST", "/")

"""
 The ListDomains operation lists all domains associated with the Access Key ID. It returns domain names up to the limit set by MaxNumberOfDomains. A NextToken is returned if there are more than MaxNumberOfDomains domains. Calling ListDomains successive times with the NextToken provided by the operation returns up to MaxNumberOfDomains more domain names with each successive operation call. 
"""
ListDomains() = simpledb("POST", "/")
...
"""
PutAttributes(DomainName, ItemName, Attributes) = simpledb("POST", "/")
...
"""
Select(SelectExpression) = simpledb("POST", "/")
...
"""
CreateDomain(DomainName) = simpledb("POST", "/")
...
"""
BatchPutAttributes(DomainName, Items) = simpledb("POST", "/")
end
