...
using .AWSCorePrototypeServices: sns

"""
List all tags added to the specified Amazon SNS topic. For an overview, see Amazon SNS Tags in the Amazon Simple Notification Service Developer Guide.
"""
ListTagsForResource(ResourceArn) = sns("POST", "/")

"""
Removes a statement from a topic's access control policy.
"""
RemovePermission(TopicArn, Label) = sns("POST", "/")

"""
Allows a topic owner to set an attribute of the topic to a new value.
"""
SetTopicAttributes(TopicArn, AttributeName) = sns("POST", "/")
...
"""
GetEndpointAttributes(EndpointArn) = sns("POST", "/")

"""
Returns all of the properties of a subscription.
"""
GetSubscriptionAttributes(SubscriptionArn) = sns("POST", "/")
...
"""
CreatePlatformApplication(Name, Platform, Attributes) = sns("POST", "/")
...
"""
CheckIfPhoneNumberIsOptedOut(phoneNumber) = sns("POST", "/")
...
"""
SetSMSAttributes(attributes) = sns("POST", "/")
...
"""
SetEndpointAttributes(EndpointArn, Attributes) = sns("POST", "/")

"""
Deletes a topic and all its subscriptions. Deleting a topic might prevent some messages previously sent to the topic from being delivered to subscribers. This action is idempotent, so deleting a topic that does not exist does not result in an error.
"""
DeleteTopic(TopicArn) = sns("POST", "/")
...
"""
TagResource(ResourceArn, Tags) = sns("POST", "/")
...
"""
CreatePlatformEndpoint(PlatformApplicationArn, Token) = sns("POST", "/")

"""
Prepares to subscribe an endpoint by sending the endpoint a confirmation message. To actually create a subscription, the endpoint owner must call the ConfirmSubscription action with the token from the confirmation message. Confirmation tokens are valid for three days. This action is throttled at 100 transactions per second (TPS).
"""
Subscribe(TopicArn, Protocol) = sns("POST", "/")

"""
Remove tags from the specified Amazon SNS topic. For an overview, see Amazon SNS Tags in the Amazon SNS Developer Guide.
"""
UntagResource(ResourceArn, TagKeys) = sns("POST", "/")
...
"""
ListPlatformApplications() = sns("POST", "/")
...
"""
ListSubscriptionsByTopic(TopicArn) = sns("POST", "/")
...
"""
OptInPhoneNumber(phoneNumber) = sns("POST", "/")
...
"""
GetPlatformApplicationAttributes(PlatformApplicationArn) = sns("POST", "/")

"""
Returns all of the properties of a topic. Topic properties returned might differ based on the authorization of the user.
"""
GetTopicAttributes(TopicArn) = sns("POST", "/")
...
"""
SetPlatformApplicationAttributes(PlatformApplicationArn, Attributes) = sns("POST", "/")
...
"""
ListEndpointsByPlatformApplication(PlatformApplicationArn) = sns("POST", "/")

"""
Allows a subscription owner to set an attribute of the subscription to a new value.
"""
SetSubscriptionAttributes(SubscriptionArn, AttributeName) = sns("POST", "/")
...
"""
ListPhoneNumbersOptedOut() = sns("POST", "/")
...
"""
Publish(Message) = sns("POST", "/")

"""
Adds a statement to a topic's access control policy, granting access for the specified AWS accounts to the specified actions.
"""
AddPermission(TopicArn, Label, AWSAccountId, ActionName) = sns("POST", "/")
end
